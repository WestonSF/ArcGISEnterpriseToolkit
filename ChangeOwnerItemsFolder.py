#-------------------------------------------------------------
# Name:       Change Owner of Items in Folder
# Purpose:    Change owner of all items in a folder in ArcGIS Online or Portal. 
...
def mainFunction(portalUrl,portalAdminName,portalAdminPassword,folderName,newOwner,newFolder,itemAccess,groupID): # Get parameters from ArcGIS Desktop tool by seperating by comma e.g. (var1 is 1st parameter,var2 is 2nd parameter,var3 is 3rd parameter)  
    try:
        # --------------------------------------- Start of code --------------------------------------- #     
        ...
        if folderID:
            ...
            getItemsFolder(portalUrl,portalAdminName,folderID,itemAccess,newOwner,newFolder,groupID,token)
        else:
...
def getItemsFolder(portalUrl,portalAdminName,folderID,itemAccess,newOwner,newFolder,groupID,token):
    dict = {}
    dict['f'] = 'json'
    dict['token'] = token    
    # Python version check
    ...
    if "error" in responseJSON:
        ...
        for item in responseJSON["items"]:
            ...
            # Changing owner name
            printMessage("Changing owner of item: " + item["id"] + " - " + newOwner + "...","info")
            reassignItem(portalUrl,portalAdminName,folderID,item["id"],newOwner,newFolder,token)
...
# Start of Reassign item function
def reassignItem(portalUrl,portalAdminName,folderID,itemId,newOwner,newFolder,token):
    # Setup parameters for request 
    dict = {}
    dict['f'] = 'json'
    dict['token'] = token
    dict['targetUsername'] = newOwner
    dict['targetFolderName'] = newFolder
    ...
    requestURL = urllib2.Request(portalUrl + "/sharing/rest/content/users/" + portalAdminName + "/" + folderID + "/items/" + itemId + "/reassign",params)
    response = urllib2.urlopen(requestURL)
    # Python version check
    if sys.version_info[0] >= 3:
        # Python 3.x
