#-------------------------------------------------------------
# Name:       Change Text In Items in Folder
# Purpose:    Does a search and replace of specified a text string
#             of all items in a folder in ArcGIS Online or Portal. 
...
def mainFunction(portalUrl,portalAdminName,portalAdminPassword,folderName,textReplace,textReplaceWith): # Get parameters from ArcGIS Desktop tool by seperating by comma e.g. (var1 is 1st parameter,var2 is 2nd parameter,var3 is 3rd parameter)  
    try:
        # --------------------------------------- Start of code --------------------------------------- #     
        ...
        if folderID:
            ...
            getItemsFolder(portalUrl,portalAdminName,folderID,textReplace,textReplaceWith,token)
        else:
...
def getItemsFolder(portalUrl,portalAdminName,folderID,textReplace,textReplaceWith,token):
    dict = {}
    dict['f'] = 'json'
    dict['token'] = token    
    # Python version check
    ...
    if "error" in responseJSON:
        ...
        for item in responseJSON["items"]:
            # Changing owner name
            printMessage("Replacing text in item: " + item["id"] + " - " + textReplaceWith + "...","info")
            updateText(portalUrl,portalAdminName,folderID,item["id"],textReplace,textReplaceWith,token)
...
def updateText(portalUrl,portalAdminName,folderID,itemId,textReplace,textReplaceWith,token):
    ...
    if "error" in responseJSON:
        ...
        # Get the current summary, description and license info
        if (responseJSON["item"]["snippet"]):
            currentSummary = responseJSON["item"]["snippet"]
        else:
            currentSummary = ""
        if (responseJSON["item"]["description"]):
            currentDescription = responseJSON["item"]["description"]
        else:
            currentDescription = ""
        if (responseJSON["item"]["licenseInfo"]):
            currentLicenseInfo = responseJSON["item"]["licenseInfo"]
        else:
            currentLicenseInfo = ""

        # If text to replace is in summary or description
        if ((textReplace in currentSummary) or (textReplace in currentDescription) or (textReplace in currentLicenseInfo)): 
            # Update the summary and description
            newSummary = currentSummary.replace(textReplace, textReplaceWith)
            newDescription = currentDescription.replace(textReplace, textReplaceWith)
            newLicenseInfo = currentLicenseInfo.replace(textReplace, textReplaceWith)
            # Setup parameters for request 
            dict = {}
            dict['f'] = 'json'
            dict['token'] = token
            dict['snippet'] = newSummary
            dict['description'] = newDescription.encode('utf-8')
            dict['licenseInfo'] = newLicenseInfo.encode('utf-8')
            ...
            if "error" in responseJSON:
                ...
                printMessage(itemId + " updated...","info")
        else:
            printMessage("Text is not present in the summary, description or license info for this item...","warning")
